...
function getCommentRange({ node, src }) {
  ...
  const commentLine = src.lines[node.source.start.line - 1];

  const isTextBeforeComment = commentLine
    .slice(0, commentLoc.start.column)
    .trim();
  // const isTextAfterComment = commentLine.slice(commentLoc.end.column, 0).trim();

  if (!isTextBeforeComment) {
    commentLoc.start.column = 0;
    commentLoc.end.column += 1;
  }

  const commentStartIdx = src.getIndexFromLoc(commentLoc.start);
  const commentEndIdx = src.getIndexFromLoc(commentLoc.end);

  return { startIdx: commentStartIdx, endIdx: commentEndIdx };
}
...
module.exports = {
  meta: {
    ...
    schema: [],
    docs: {
      description: "Styled Component should not have comments in their css",
      recommended: true,
